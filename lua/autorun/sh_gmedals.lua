...
if SERVER then 
    ...
    util.AddNetworkString("GMEDAL_GROUP_CREATE")
    util.AddNetworkString("GMEDAL_GROUP_REMOVE")
end
...
function ReloadGroups()
    if  not file.Exists('gmedals-groups-config.json', 'DATA') then
        file.Write('gmedals-groups-config.json', util.TableToJSON(gMedals.Groups,true))
        tbl = util.JSONToTable(file.Read('gmedals-groups-config.json', 'DATA'))
    else
        tbl = util.JSONToTable(file.Read('gmedals-groups-config.json', 'DATA'))
    end
    return tbl
...
net.Receive('RELOAD_CONFIG', function()
    ...
    gMedals.Groups = ReloadGroups()
end)

net.Receive('GMEDAL_GROUP_CREATE', function()
    ...
    table.Add(gMedals.Groups, newGroup)
    local TblToAdd = util.TableToJSON(gMedals.Groups, true)

    file.Write("gmedals-groups-config.json", TblToAdd)
    net.Start('RELOAD_CONFIG')
    net.Broadcast()
end)

net.Receive('GMEDAL_GROUP_REMOVE', function()
    gMedals.Groups = ReloadGroups()  
    local groupName = net.ReadString()  

    local groupIndexToRemove = nil
    for k, v in pairs(gMedals.Groups) do
        if v.name == groupName then
            groupIndexToRemove = k
            break  
        end
    end
    if groupIndexToRemove then
        table.remove(gMedals.Groups, groupIndexToRemove)
        print("Group removed: ", groupName)
        local updatedTableJSON = util.TableToJSON(gMedals.Groups, true)
        file.Write("gmedals-groups-config.json", updatedTableJSON)
    else
...
if  not file.Exists('gmedals-groups-config.json', 'DATA') then
    file.Write('gmedals-groups-config.json', util.TableToJSON(gMedals.DefaultGroups,true))
    gMedals.Groups = util.JSONToTable(file.Read('gmedals-groups-config.json', 'DATA'))
else
    gMedals.Groups = util.JSONToTable(file.Read('gmedals-groups-config.json', 'DATA'))
end
