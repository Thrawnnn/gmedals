...
--	DermaPanel:SetDraggable(true)
    ...
    local group = vgui.Create("Nexus:ComboBox", Scroll)
    group:SetPos( ScrW() / 75, ScrH() / 4.7 )
    group:SetSize( ScrW() / 13, ScrH() / 35 )
    group:SetValue( "Medal Group" )
    group:AddChoice('Other')
    
    for k,v in ipairs(gMedals.Groups) do
        group:AddChoice(v.name)
    end

    group.OnSelect = function( _, _, value )
        if value == "Other" then
            Derma_StringRequest(
                "Medal Group Creation", 
                "Please Choose the name of your new Group!",
                "",

                function(text)
                    Derma_StringRequest(
                        "Medal Group Creation", 
                        "Please Choose the Material of your new Group!",
                        "",
                
                        function(mat)
                            net.Start("GMEDAL_GROUP_CREATE")
                                net.WriteString(text)
                                net.WriteString(mat)
                            net.SendToServer()
                            DermaPanel:Remove()
                            notification.AddLegacy("New medal group created: "..text, 0, 5)
                            end,
    ...
    ResetButton.DoClick = function()
        ...
        Derma_Query(
            ...
            function()
                ...
                net.SendToServer()
                notification.AddLegacy("Configuration Reset successfully.", 0, 5)
                DermaPanel:Remove()
            end,
            "No",
    ...
    comboBox:SetPos( ScrW() / 8, ScrH() / 6.75 )			
    comboBox:SetSize( ScrW() / 10, ScrH() / 35 )
    comboBox:SetValue( "Destroy Medal" )

    local group = vgui.Create("Nexus:ComboBox", Scroll)
    group:SetPos( ScrW() / 8, ScrH() / 4.7 )
    group:SetSize( ScrW() / 13, ScrH() / 35 )
    group:SetValue( "Destroy Medal Group" )
    ...
    group.OnSelect = function( _, _, value )
        ...
        Derma_Query(
            ...
            "Yes",
            function() 
                net.Start("GMEDAL_GROUP_REMOVE")
                    net.WriteString(value)
                net.SendToServer()
                DermaPanel:Remove()
                notification.AddLegacy("Medal Group removed:  "..value, 1, 5)
            end,
            "Cancel",
            function()
                notification.AddLegacy("Action Cancelled.", 1, 5)
            end
        )
    ...
    comboBox.OnSelect = function( _, _, value )
        ...
        Derma_Query(
            ...
            function()
                ...
                net.SendToServer()
                DermaPanel:Remove()
                notification.AddLegacy("Medal Removed successfully.", 0, 5)
            end,
            "No",
            function() return notification.AddLegacy("Action Cancelled", 0, 5) end
        )
    ...
    ConfigButton:SetPos( ScrW() / 75, ScrH() / 3.85 )					
    ...
    ConfigButton.DoClick = function()
        ...
        local groupVal = groupValue or "Default"
        DermaPanel:Remove()
        net.Start('ADD_NEW_MEDAL')
            ...
            net.WriteString(groupVal)
        net.SendToServer()

        notification.AddLegacy("Exported successfully!", 0, 5)
    end
